...
function TooltipShadcn({ ...props }: TooltipPrimitive.Root.Props) {
...
function Tooltip({
  ...
  return (
    <TooltipShadcn open={open} onOpenChange={onOpenChange} defaultOpen={defaultOpen} disabled={disabled}>
      {isValidElement(children) ? children : <TooltipTrigger className={className}>{children}</TooltipTrigger>}
      {typeof content === 'string' ? (
        <TooltipContent className={contentClassName} dangerouslySetInnerHTML={{ __html: content }} />
      ) : (
        <TooltipContent
          side={side}
          sideOffset={sideOffset}
...
export { TooltipShadcn, TooltipTrigger, TooltipContent, TooltipProvider, Tooltip };